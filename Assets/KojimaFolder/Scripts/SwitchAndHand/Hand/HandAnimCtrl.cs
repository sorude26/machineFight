...
public class HandAnimCtrl : MonoBehaviour
{
    ...
    [SerializeField]
    private SwitchCtrlHand m_myHand = null;
    [SerializeField]
    private HandPoseWeights m_defaultGrabPose;
    ...
    private void Update()
    {
        ...
        float flex = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_myHand.ControllerType);
    ...
    {
        m_isPointing = !OVRInput.Get(OVRInput.NearTouch.PrimaryIndexTrigger, m_myHand.ControllerType);
        m_isGivingThumbsUp = !OVRInput.Get(OVRInput.NearTouch.PrimaryThumbButtons, m_myHand.ControllerType);
    }
    ...
    {
        bool grabbing = m_myHand.State == SwitchCtrlHand.HandState.LockIn;
        HandPoseWeights grabPose = m_defaultGrabPose;
        if (grabbing)
        {
            grabPose = m_myHand.LockingSwitch.GetPose();
        }
        // Pose
        ...
        float flex = OVRInput.Get(OVRInput.Axis1D.PrimaryHandTrigger, m_myHand.ControllerType);
        flex = grabPose.GetFlex(flex);
        m_animator.SetFloat(m_animParamIndexFlex, flex);

        // Point
        float point = grabPose.GetPoint(m_pointBlend);
        m_animator.SetLayerWeight(m_animLayerIndexPoint, point);

        // Thumbs up
        float thumbsUp = grabPose.GetThumbsUp(m_thumbsUpBlend);
        m_animator.SetLayerWeight(m_animLayerIndexThumb, thumbsUp);

        // Pinch
        float pinch = OVRInput.Get(OVRInput.Axis1D.PrimaryIndexTrigger, m_myHand.ControllerType);
        pinch = grabPose.GetPinch(pinch);
